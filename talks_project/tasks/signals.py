...
def pre_save_task(sender, instance, **kwargs):
    if instance.id:
        ...
        if previous.title != instance.title:
            ...
            status_ = f"priority changed to {instance.description}"
        ...
        data = {
            ...
            "priority": instance.priority,
            "status": instance.status,
            "status_": f"create a task with title name {instance.title}"
        }
        async_to_sync(channel_layer.group_send)(
            "live_data_live_data" ,
            {
...
def post_delete_task_notification(sender, instance, **kwargs):
    data = {
        ...
        "priority": instance.priority ,
        "status": instance.status ,
        "status_": f"deleted the {instance.title}"
    }
    async_to_sync(channel_layer.group_send)(
        "live_data_live_data",
        {
...
def post_delete_task_notification(sender, instance, **kwargs):
    data = {
        "comment": instance.content ,
        "status_": f"comment for the task title '{instance.task.title}' has deleted"
    }
    async_to_sync(channel_layer.group_send)(
        "live_data_live_data",
        {
